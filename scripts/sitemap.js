...
(async () => {
    const pages = await globby([
        ...
        "!./pages/_*.tsx",
        "!./pages/[*.tsx",
        "!./pages/_blog-admin/**/*.tsx",
    ]);
    const markdowns = await globby(["./mds/*"]);
  ...
  ${markdowns
        .map((markdown) => {
            const path = markdown.replace("./mds/", "").replace(".md", "");
            return `<url>
  ...
  <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
    ...
    writeFileSync("./public/sitemap.xml", formattedSitemap, "utf-8");
})();
